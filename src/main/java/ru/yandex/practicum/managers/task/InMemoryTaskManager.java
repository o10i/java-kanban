...
import ru.yandex.practicum.managers.exceptions.IntersectionTaskException;
import ru.yandex.practicum.managers.exceptions.NoTaskException;
...
*/
    ...
    public void determineEpicStatus(int id) {
        if (getEpicSubtasks(id).stream().allMatch(subtask -> subtask.getStatus().equals(Status.NEW))) {
            epicMap.get(id).setStatus(Status.NEW);
        } else if (getEpicSubtasks(id).stream().allMatch(subtask -> subtask.getStatus().equals(Status.DONE))) {
